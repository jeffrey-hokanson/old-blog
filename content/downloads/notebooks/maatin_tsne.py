...
def x2p(X = Math.array([]), tol = 1e-5, perplexity = 30.0):
	...
	for i in range(n):
		...
		while Math.abs(Hdiff) > tol and tries < 50:
			...
			# JMH: There was a bug preventing convergence of the binary search
			# betamin = beta[i] => betamin = float(beta[i])
			# Some numpy memory issue
			if Hdiff > 0:
			...
			(H, thisP) = Hbeta(Di, beta[i]);
			Hdiff = H - logU;
			print (betamin,float(beta[i]) , betamax,  float(Hdiff))
			tries = tries + 1;
